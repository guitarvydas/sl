...
(esrap:defrule <unparse-body> (or <token-emit-kind> <push> <get-field> <foreach-in-list> <foreach-in-table> <unparse-call-rule> <unparse-call-external>)) 
...
(esrap:defrule <foreach-in-table> (and "$" "foreach" (* <ws>) LBRACE (+ <unparse-body>) RBRACE)
  (:destructure (tilde foreach ws lb body rb)
   ...
   (let ((key (gensym)))
     `(let ()
        (unparse-foreach-in-table #'(lambda () ,(append-lets body)))))))
...
(esrap:defrule <foreach-in-list> (and "~" "foreach" (* <ws>) LBRACE (+ <unparse-body>) RBRACE)
  (:destructure (tilde foreach ws lb body rb)
   ...
   (let ((key (gensym)))
     `(let ()
        (unparse-foreach-in-list #'(lambda () ,(append-lets body)))))))
